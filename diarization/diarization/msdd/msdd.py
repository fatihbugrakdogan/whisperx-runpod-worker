...
class MSDDDiarizer:
    # Domain default: psikoterapi seansları her zaman 2 konuşmacılı (terapist +
    # danışan). NeMo'nun ClusteringDiarizer'ı num_speakers=None ile kümeleri
    # otomatik bulmaya bırakıldığında 2 kişilik konuşmalarda sık sık over-
    # cluster ediyor (terapistin tonlamalarını ayrı konuşmacı sanıyor). Sabit
    # 2 kümele constraint'i bu over-clustering'i ortadan kaldırır.
    #
    # 2-konuşmacı olmayan bir senaryo gerekirse `__init__(num_speakers=...)`
